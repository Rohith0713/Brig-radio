...
class Radio(db.Model):
    ...
    accumulated_duration = db.Column(db.Integer, default=0) # In seconds
    ...
    def to_dict(self, user_id=None):
        ...
        # Calculate current live duration if it's hosting
        if self.host_status == HostStatus.HOSTING and self.last_resumed_at:
            elapsed = (datetime.now() - self.last_resumed_at).total_seconds()
            result['current_duration_seconds'] = int(self.accumulated_duration + elapsed)
        else:
            result['current_duration_seconds'] = self.accumulated_duration
