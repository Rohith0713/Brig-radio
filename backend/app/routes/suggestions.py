...
def get_my_suggestions():
    """Get student's own suggestions"""
    user_id = int(get_jwt_identity())
    
    suggestions = RadioSuggestion.query.filter_by(
        suggested_by=user_id
    ).order_by(RadioSuggestion.created_at.desc()).all()
...
def delete_suggestion(suggestion_id):
    """Delete a suggestion (student only, own suggestions)"""
    user_id = int(get_jwt_identity())
    ...
    # Check ownership
    if suggestion.suggested_by != user_id:
        return jsonify({'error': 'Unauthorized to delete this suggestion'}), 403
